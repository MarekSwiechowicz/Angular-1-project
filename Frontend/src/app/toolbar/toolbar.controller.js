(function ()
{
    ...
    /** @ngInject */
    function ToolbarController($rootScope, $q, $state, $timeout, $mdSidenav, $translate, $mdToast, msNavigationService, AuthService, $scope, $location)
    {
        var vm = this;
        ...
        function getUserName(){
          ...
          return AuthService.loggedUser.name;
        }
        ...
        function tryLogin(){
            AuthService.login(vm.login.email, vm.login.password).then(function(loggedIn){
              vm.login = {
                email: '',
                password: ''
              };
              if(loggedIn){
                if($state.current.name!=='app.auth_register'){
                  $state.reload();
                }
                else{
